...
class ServerRequest(object):
    def __init__(self, who_req, the_req):
        self.requester = who_req
        self.action = the_req
...
class ServerNet(ServerNetworking):
    ...
    def handle_tasks(self,payload):
        ...
        request = ServerRequest(sid,decoder(msg[2:]))
        self.request_queue.append(request)

    def disconnect(self):
        super.close()
    
    def send_reply(self, reply:ServerRequest):
        if self.listener is not None:
            if self.listener.is_stopped():
                raise RuntimeError("Connection has been stopped...",self.listener.reason)
            msg = reply.get_payload()
            try:
                self.reply(msg[0],msg[1:])
            except Exception as e:
                import traceback
                self.stop("\n".join(['-MAIN THREAD ERROR-',traceback.format_exc()]))
                raise RuntimeError("Could not handle message")
            return True
        return None


class ServerState(object):
    def __init__(self, root_dir, network_interface:ServerNet, uhd_args, log_template, debug, use_log=True):
        self.radios         = None
        self.root           = root_dir
        self.save_dir       = None
...
command_list=["help","ping","get_radios","get_active","get_finished",
              ...
              "run_script"]
def run(network_interface:ServerNet,uhd_args=[],debug=False,root_dir='/data/local/wfgen_reports',use_log=True):
    """
    ...
    def sigint_handle(sig,frame):
        ...
        network_interface.request_queue.appendleft(ServerRequest(None,["shutdown","quiet"]))
        msg_waiting = False
        signal.signal(signal.SIGINT, signal.default_int_handler)
    ...
    while True:
        ...
        if msg_waiting:
            ...
            if cmd not in command_list:
                if not no_reply:
                    network_interface.send_reply(ServerRequest(sent_from,['Invalid command:',cmd]))
            else:
                if cmd == 'shutdown':
                    clean_up()
                    Current_STATE.consolidate()
                    if message[1] != 'quiet':
                        network_interface.send_reply(ServerRequest(sent_from,["Shutting","down"] + ["{0!s}".format(x) for x in finished_procs]))
                    for x in network_interface.state_map.keys():
                        rep = ServerRequest(x,["bye","bye"])
                    ...
                    if not no_reply:
                        network_interface.send_reply(ServerRequest(sent_from,['Found:\n',info]))
                    ...
                    if(len(args_index) == 0):
                        if any([x == '__debug_test__' for x in message]):
                            ...
                            try:
                                ...
                                if not no_reply:
                                    network_interface.send_reply(ServerRequest(sent_from,["Starting process","{}".format(proc)]))
                                active_procs.append(([proc,dummy_proc(proc)," ".join(message),[-1]]))
                            except:
                                if not no_reply:
                                    network_interface.send_reply(ServerRequest(sent_from,["Unable","to","start","process"]))
                        else:
                            if not no_reply:
                                network_interface.send_reply(ServerRequest(sent_from,["Invalid","command","or","unknown","command"]))
                    else:
                        ...
                        if radio_index == -1:
                            ...
                            ########### need to still reply
                            if not no_reply:
                                network_interface.send_reply(ServerRequest(sent_from,["not my radio ->","{}".format(dev_serial)]))
                        else:
                            proc = None
                            ...
                            if proc is None:
                                ...
                                if not no_reply:
                                    network_interface.send_reply(ServerRequest(sent_from,invalid_request))
                            elif isinstance(proc,subprocess.Popen):
                                if not no_reply:
                                    network_interface.send_reply(ServerRequest(sent_from,["Starting process","{}".format(proc.pid),truth_file]))
                                ...
                                if not no_reply:
                                    network_interface.send_reply(ServerRequest(sent_from,["Starting process","<{0!s} & {1!s}>".format(
                    ...
                    for kill_slot in message[1:]:
                        ...
                        if int(kill_slot) in active_pids:
                            ...
                            if not no_reply:
                                # network_interface.send_reply(ServerRequest(sent_from,["Killing","process","{}".format(message[1])]))
                            ...
                            if not no_reply:
                                # network_interface.send_reply(ServerRequest(sent_from,["No","process","{}".format(message[1])]))
                                cleanup_messages.append("No process {}".format(message[1]))
                    if cleanup_messages:
                        reply = "\n".join(cleanup_messages)
                        network_interface.send_reply(ServerRequest(sent_from,reply.split(' ')))
                elif cmd == 'ping':
                    if not no_reply:
                        network_interface.send_reply(ServerRequest(sent_from,['pong']))
                elif cmd == 'get_active':
                    if len(active_procs) == 0:
                        if not no_reply:
                            network_interface.send_reply(ServerRequest(sent_from,["No","Active","Processes"]))
                    else:
                        if not no_reply:
                            network_interface.send_reply(ServerRequest(sent_from,["{0!s}".format(x) for x in active_procs]))
                elif cmd == 'get_finished':
                    if len(finished_procs) == 0:
                        if not no_reply:
                            network_interface.send_reply(ServerRequest(sent_from,["No","Finished","Processes"]))
                    else:
                        if not no_reply:
                            network_interface.send_reply(ServerRequest(sent_from,["{0!s}".format(x) for x in finished_procs]))
                elif cmd == 'run_random':
                    try:
                        ...
                        if not no_reply:
                            network_interface.send_reply(ServerRequest(sent_from,response))
                    if response is None:
                        ...
                        if proc is None:
                            ...
                            if not no_reply:
                                network_interface.send_reply(ServerRequest(sent_from,response.split()))
                        elif isinstance(proc,list) and len(proc) > 0 and isinstance(proc[0],str):
                            if not no_reply:
                                network_interface.send_reply(ServerRequest(sent_from,proc)) ## something went wrong
                        elif isinstance(proc,mp.Process):
                            if not no_reply:
                                network_interface.send_reply(ServerRequest(sent_from,["Starting random run:","{}".format(proc.pid)]))
                    ...
                    if report is None:
                        network_interface.send_reply(ServerRequest(sent_from,['report','empty','']))
                        continue
                    report_status = 'valid'
                    try:
                    ...
                    if report_status != 'empty':
                        ...
                        report_contents = ''
                    network_interface.send_reply(ServerRequest(sent_from,['report',report_status,report_contents]))
                    ...
                    if response is None:
                        ...
                        if len(valid_radio_args) == 0:
                            ...
                            if proc is None:
                                ...
                                if not no_reply:
                                    network_interface.send_reply(ServerRequest(sent_from,["Starting scripted run:","{}".format(proc.pid)]))
                                active_procs.append((proc.pid,proc," ".join(message),radios))
                else:
                    if not no_reply:
                        network_interface.send_reply(ServerRequest(sent_from,["Valid commands:"]+command_list))
        msg_waiting = False
        no_reply = False
...
def parse_args():
    ...
    p.add_argument("--uhd-args",default=[],type=str,action='append',
        help="Limit to devices whose flag provided will find (def: all uhd devices)")
    ...
    if args.make_default_net:
        ...
        import json
        with open("default_server_network.json","w") as fp:
...
def main():
    args = parse_args()
    network = ServerNet(args.net_config)
    try:
