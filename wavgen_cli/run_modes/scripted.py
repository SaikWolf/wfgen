...
try:
    ...
    from data_gen_remote_listener.profiles import get_all_profile_names,extract_profile_by_name,get_replay_profile_names
...
def parse_script_request(request:Dict,server_state:"ServerState",use_log=False):
    ...
    '''
    import pprint
    from data_gen_remote_listener.profiles import get_all_profile_names
    ...
    if 'toggles' in request:
        ...
        toggles = dict()
    # if flags in [1,2]:
    #     from data_gen_remote_listener.profiles import get_traceback_profiles
        ...
        if flags in [1,2,3]:
            ...
            for sig_info in request[key][1:]: ### all signal requests
                ...
                if 'extras' in sig:            # delete these on the first pass through||| not needed yet
                    del sig['extras']
                # sig['profile'] = sig['dgrl_profile_name']
...
def translate_config(config_a:Dict,rng:RNG_TYPE,log_c=None) -> Dict:
    from data_gen_remote_listener import c_logger
    # import pprint
...
def debug_server_scripting(uhd_args=[],script='truth_scripts/output-1a-truth.json'):
    ...
    from data_gen_remote_listener.server import ServerState
    from data_gen_remote_listener.profiles import get_traceback_profiles
    from data_gen_remote_listener.utils import get_interface
    server = ServerState('/tmp',get_interface(),50000,'172.21.192.86',56000,uhd_args,None,False,False)
    from data_gen_remote_listener.launcher import get_radios
