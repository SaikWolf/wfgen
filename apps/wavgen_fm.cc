...
int main (int argc, char **argv)
{
    ...
    uint64_t n(8192);
    uint8_t itemsize = 1;///////complex float is default size
    ...
    uint64_t wav_offset;
    uint64_t counter = 0;
    double bandwidth,baud_rate,max_freq; // not actually in use yet
    if(modulation == analog_source_type2str(CONSTANT)){
        ...
        bandwidth = 0.001*uhd_tx_rate;              // this is actually the TX bandwidth of FM, not real_path value
        baud_rate = -1;
        max_freq = -1;
    }
        ...
        wrap_source = real_source_create(gen_peek,sizeof(float),1,NONE);
        bandwidth = uhd_tx_rate/2*mod_index;          // this is actually the TX bandwidth of FM, not real_path value
        baud_rate = -1;
        max_freq = -1;
    }
        ...
        itemsize = wav->itemsize/sizeof(std::complex<float>);//bytes per sample -----> should be 1 always, but sanity check.
        ...
        gen_peek = (void*) rand_gauss_source_create();
        wrap_source = real_source_create(gen_peek,sizeof(float),1,NONE);
        bandwidth = uhd_tx_rate/2*mod_index;          // this is actually the TX bandwidth of FM, not real_path value
        baud_rate = -1;
        max_freq = -1;
        ...
        gen_peek = (void*) rand_uni_source_create();
        wrap_source = real_source_create(gen_peek,sizeof(float),1,NONE);
        bandwidth = uhd_tx_rate/2*mod_index;          // this is actually the TX bandwidth of FM, not real_path value
        baud_rate = -1;
        max_freq = -1;
    ...
    real_path rp = real_path_create(uhd_tx_rate,uhd_tx_rate,// in and out are the same, no resampling
                    bandwidth, // expected bandwidth
                    max_freq,  // exepected max frequency
                    baud_rate, // expected baud_rate
                    1.0, // digital gain (just leave as 1.0, must be <=1.0, greate will clip)
                    0.0, // frequency offset to apply
                    wrap_source);// where to pull the real stream from
    amgen am = amgen_create(1, 1.0, NULL, &rp); // all the paths that need to be generated
    fmgen fm = fmgen_create(mod_index,am); // fm modulate the am signal
    ...
    uint64_t samples = buf.size()*counter;
